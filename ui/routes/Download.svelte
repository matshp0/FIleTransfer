<script>
  ...
  let writer = {};
  let files = [];
  let filesGenerator = generator(files);
  const download = downloader.startDownload.bind(downloader);

  const nextFile = () => {
    const { value, done } = filesGenerator.next();
    if (done) {
